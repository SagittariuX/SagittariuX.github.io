...
$("#navigationSection ul li a").click(function () {
  ...
  if ($(section).scrollTop() < $(window).scrollTop()) {
    // scrolling to a previous section
    hideNav();
    $("nav").addClass("manual");
    setTimeout(() => {
    ...
    $("#experience").append(`
            <div class="experienceItem wow fadeInUp">
                <div class="experiencePictureFrame">
                    <img class="experiencePic" src="${
                      picuri + json.items[$i].pic
                    }">
                </div>
            
                <div class="experienceText">
                    <h3 class="experiencePosition">${
                      json.items[$i].position
                    }</h3>
                    <div>
                        ${json.items[$i].text}
                    </div>
    ...
    $("#projects").append(`
            <div class="projectItem wow fadeInUp">
                <div class="projectPictureFrame" >
                    <img class="${json.items[$i].orientation}" src="${
      picuri + json.items[$i].pic
    }">
                </div>
...
function formatTracks(focus, handlerSource = null) {
  ...
  if (focus > -1 && focus < $(".spotifyTrack").length) {
    //make sure nothing is out of index
    $gFocusTrack = focus;
  } else {
    return;
  }
...
function changeAudio(focus, player, handlerSource = null) {
  ...
  if (handlerSource) {
    //only play the song if user explicitly commands
    player.play();
  }
  return;
}
